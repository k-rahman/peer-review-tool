...
namespace Emailing.Service.API.Events.EventHandlers
{
	public class ReviewStartedEventHandler : IConsumer<ReviewStarted>
	{
		...
		public ReviewStartedEventHandler(IMapper mapper, ManagementApiClient managementApiClient, IMessagingService messagingService)
		{
			_mapper = mapper;
		...
		public async Task Consume(ConsumeContext<ReviewStarted> context)
		{
			...
			foreach (var participant in participants)
			{
				...
				if (verified)
				{
					var emailSubject = "Review phase started on peer review";
				...
				{
					var emailSubject = "Reset password and review phase started on peer review";
